...
# VOICE FLOW
# 1. Frontend requests all Elevenlabs voices available and their ids, backend responds with a list of voice id - name mappings
# 2. Frontend receives the list of voice id - name mappings, renders to customer to select from the dropdown.
# 3. customer selects voice name, Frontend sends selected voice id to backend , and backend generates voice preview with the input text, return base64 audio
# 4. customer on frontend selects and saves the voice id for the agent in agent test, backend updates the agent voice id setting

from services.elevenlabs_service import ElevenLabsServiceSynchronousForAgentVoicePreview

@router.get("/voices", response=List[VoiceOption], auth=auth)
def get_available_voices(request, force_refresh: bool = False):
    """
    Get all available ElevenLabs voices
    
    Args:
        force_refresh: Force fetching from API instead of cache
    """
    elevenlabs_voice_service = ElevenLabsServiceSynchronousForAgentVoicePreview()
    voices = elevenlabs_voice_service.get_available_voices(force_refresh=force_refresh)
    return [VoiceOption(**voice) for voice in voices]

@router.post("/{agent_id}/preview-voice", auth=auth)
def preview_agent_voice(request, agent_id: str, data: VoicePreviewRequest):
    """Generate a voice preview for the agent"""
    
    if not data.text:
        return 400, {"error": "Text is required for voice preview"}
    
    if not data.voice_id:
        return 400, {"error": "Voice ID is required for voice preview"}
    ...
    try:
        ...
        elevenlabs_voice_service = ElevenLabsServiceSynchronousForAgentVoicePreview()
        audio_base64 = elevenlabs_voice_service.generate_audio(
            text=data.text,
        ...
        if not audio_base64:
            return 500, {"error": "Failed to generate audio preview"}
        ...
        return {
            ...
            'duration_seconds': len(data.text) * 0.06
        }
...
@router.patch("/{agent_id}/voice", auth=auth)
def update_agent_voice(request, agent_id: str, data: UpdateVoiceRequest):
    """Update the voice ID for an agent"""
    ...
    try:
        ...
        return {"message": "Voice updated successfully", "voice_id": agent.voice_id}
        
    except Agent.DoesNotExist:
        return 404, {"error": "Agent not found"}
    except Exception as e:
        logger.error(f"Failed to update agent voice: {e}")
        return 400, {"error": str(e)}
...
# # TEST AGENT Voice Preview Endpoint
# @router.post("/{agent_id}/preview-voice", auth=auth)
...
def test_agent_chat(request, agent_id: str, data: TestChatRequest):
    ...
    try:
        ...
        from services.ai_service import AIService_for_test_agent
        try:
            ai_service = AIService_for_test_agent()
            ai_response, tokens_used = ai_service.generate_response(
                message=message,
                agent=agent,
                conversation_history=conversation_history
            )
            logger.info(f"TESTING CHAT AGENT {agent.id} AI Service response: {ai_response}, Tokens used: {tokens_used}")
